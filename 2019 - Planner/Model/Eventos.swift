...
class Evento {
    ...
    public func selfDelete() {
        self.done = true
        // duvidas por aqui, Sr. Enzo
        let selfIndex = (self.eventoPai?.subeventos.firstIndex(where: { $0.id == self.id }))!// compara a id do evento dentro do array do subeventos do evento pai
        self.eventoPai?.subeventos.remove(at: selfIndex)// remove do array o evento
